...
gmd(
  ...
  async (from, Guru, conText) => {
    ...
    if (!q)
      return reply(
        "❌ Please specify what to react with!\n\n" +
        "Examples:\n" +
    ...
    try {
      const input = q.trim();
      const value =
        ["random", "off", "default"].includes(input.toLowerCase()) ? "" : input;

      const current = await getSetting("STATUS_REACT_EMOJI");
      if ((current || "") === value) {
        return reply(`⚠️ Status reaction is already set to that.`);
      }

      await setSetting("STATUS_REACT_EMOJI", value);
      await react("✅");

      if (!value) {
        await reply("✅ Status reactions reset to random emoji pool.");
